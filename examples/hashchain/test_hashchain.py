...
def run():
    ...
    with open(p2, "w") as f: f.writelines(open(p).readlines())
    ls = open(p2).readlines(); obj = json.loads(ls[10]); obj["text"] = "ALTERED"
    ls[10] = json.dumps(obj) + "\n"
    open(p2, "w").writelines(ls)
    ok, det = hc.verify(p2)
    if ok or det.get("broken_at") != 10: fails.append(("T3 tamper detect", ok, det))
    # T4: delete an entry -> detected
    p3 = os.path.join(d, "deleted.jsonl"); ls = open(p).readlines(); del ls[5]
    open(p3, "w").writelines(ls)
    ok, det = hc.verify(p3)
    if ok: fails.append(("T4 delete detect", ok, det))
    # T5: reorder -> detected
    p4 = os.path.join(d, "reorder.jsonl"); ls = open(p).readlines(); ls[3], ls[4] = ls[4], ls[3]
