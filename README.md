...
> **Status: early and honest.** A working system running daily for its first user, not a finished product. **This repository ships the concept, the data shapes ([examples/vault/SCHEMA.md](examples/vault/SCHEMA.md)), the runnable hash-chain core, a minimal one-Mind runtime, and a public runtime slice ([examples/runtime](examples/runtime): roster, scheduled tick, spend-capped router, evidence-gated council), all stdlib Python with no database.** The rest of the private instance (the FastAPI server, the web app, the 35-Mind roster and voices, the Letta memory layer, the ingest pipelines) is being sanitized for public release. This README marks what works versus what is still aspirational, and the [roadmap](docs/ROADMAP.md) tracks the gap. We are building in the open on purpose.
