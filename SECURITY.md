...
- **Tampering with history.** Where the hash-chain module is used, timelines are hash-chained, so altering, deleting, or reordering existing entries within a stream is detectable (`examples/hashchain`). Rollout across every write path is in progress (see the roadmap). Protecting against truncation or wholesale replacement of a file is a further layer (off-box backup, external anchoring), still to build.
