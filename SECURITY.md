...
- **Data loss.** Sync is not backup. The canonical files are replicated to the owner's devices and pushed to a versioned, off-site, encrypted git backup, treated as disaster recovery.
- **Tampering with history.** Timelines are hash-chained, so altering, deleting, or reordering past entries is detectable (`examples/hashchain`).
- **Over-broad agent action.** Minds should not be able to do arbitrary damage. The enforcement layer routes significant writes through a validation service that refuses unauthorized deletion and malformed writes. This is partly in code and partly still convention; closing that gap is a top priority.
